...
async fn fn_delete(input: &Value) -> CallResponse {
    ...
    if !from.is_empty() && !to.is_empty() {
        ...
        // CHECK first - verify node exists before deleting
        let check_cypher = format!("MATCH (n {{id: '{}'}}) RETURN n LIMIT 1", target_id);
        let result = g.execute(query(&check_cypher)).await;
        match result {
            Ok(mut rows) => {
                if let Ok(Some(_)) = rows.next().await {
                    //EXISTS - delete node
                    if let Err(e) = g.run(query(&format!("MATCH (n {{id: '{}'}}) DETACH DELETE n", target_id))).await {
                        return CallResponse::err(e.to_string());
                    }
                    emit_log("debug", &format!("Neo4j: delete DELETED node: {}", target_id));
                } else {
                    emit_log("debug", &format!("Neo4j: delete node not found: {}", target_id));
                }
            },
            Err(e) => return CallResponse::err(e.to_string()),
        }
