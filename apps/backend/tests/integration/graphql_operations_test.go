...
func TestIntegration_Mutation_DeleteMemoryNode(t *testing.T) {
	memRepo := graphql.NewTestMemoryRepoWithState()
	// Pre-populate a node that will be returned by user_graph
	memRepo.AddNode("user:alice", "alice", "Alice content", "Alice content")

	// Pass memRepo as BOTH memoryRepo and graphRepo since it implements both interfaces
	qs := svcdashboard.NewQueryService(nil, memRepo, memRepo, nil, nil)
	cs := svcdashboard.NewCommandService(nil, memRepo, nil)
	deps := graphql.NewTestDeps(graphql.TestDepsOpts{
		Agent:      &dto.AgentSnapshot{Name: "Bot", Status: "ok"},
		QuerySvc:   qs,
	...
	// 1. Query userGraph to get the node's ID as the plugin exposes it
	resp := gqlPost(t, h, `{"query":"query { userGraph(userId: \"user:alice\") { nodes { id label } } }"}`)
	graphData := dataOf(t, resp)["userGraph"].(map[string]interface{})
	var nodes []interface{}
	if graphData["nodes"] != nil {
		nodes = graphData["nodes"].([]interface{})
	}
	require.Len(t, nodes, 1, "expected exactly one node in userGraph")
	nodeID := nodes[0].(map[string]interface{})["id"].(string)
	require.NotEmpty(t, nodeID, "node id from userGraph must not be empty")

	// 2. Delete using the ID that userGraph returned (this is what the frontend does)
	resp = gqlPost(t, h, fmt.Sprintf(`{"query":"mutation { deleteMemoryNode(id: \"%s\") }"}`, nodeID))
	assert.Equal(t, true, dataOf(t, resp)["deleteMemoryNode"], "deleteMemoryNode should succeed")

	// 3. Verify the node is actually gone (plugin must be self-consistent)
	resp = gqlPost(t, h, `{"query":"query { userGraph(userId: \"user:alice\") { nodes { id label } } }"}`)
	graphData = dataOf(t, resp)["userGraph"].(map[string]interface{})
	if graphData["nodes"] != nil {
		nodes = graphData["nodes"].([]interface{})
	...
	assert.Len(t, nodes, 0, "node should be deleted after deleteMemoryNode")
}
