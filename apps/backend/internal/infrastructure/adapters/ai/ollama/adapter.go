...
func NewAdapterWithAuth(baseURL, apiKey, model string, maxTokens int) *Adapter {
	...
	if err != nil {
		...
		return &Adapter{client: c, model: model, maxTokens: maxTokens}
	}
...
func (a *Adapter) Chat(ctx context.Context, req ports.ChatRequest) (ports.ChatResponse, error) {
	...
	log.Printf("ollama: sending request with model=%s, %d messages, %d tools",
		a.model, len(messages), len(tools))
	for i, t := range req.Tools {
		if t.Function != nil {
			log.Printf("ollama:   tool[%d] name=%q", i, t.Function.Name)
		}
	}
	for i, msg := range messages {
		...
		log.Printf("ollama:   [%d] role=%s content=%q", i, msg.Role, preview)
	}
	...
	log.Printf("ollama: done_reason=%q content_len=%d tool_calls=%d",
	...
	if len(sdkResp.Message.ToolCalls) > 0 {
		...
		log.Printf("ollama: extracted %d tool_calls (SDK), stop_reason=tool_use", len(result.ToolCalls))
	}
	...
	if len(result.ToolCalls) == 0 && strings.Contains(result.Content, "<tool>") {
		...
		if len(parsed) > 0 {
			...
			log.Printf("ollama: extracted %d tool_calls (<tool> blocks), stop_reason=tool_use", len(result.ToolCalls))
		}
	}
