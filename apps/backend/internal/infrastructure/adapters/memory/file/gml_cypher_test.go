...
func TestGMLBackend_QueryGraph_Integration(t *testing.T) {
	...
	_ = b.AddKnowledge(ctx, "user1", "User lives in Madrid", "Madrid", "LIVES_IN", nil)
	_ = b.AddKnowledge(ctx, "user1", "User likes music", "Music", "LIKES", nil)
	...
	if len(result.Data) < 2 {
		t.Errorf("expected at least 2 nodes, got %d", len(result.Data))
	}
}
