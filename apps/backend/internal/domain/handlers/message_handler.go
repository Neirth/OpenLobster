...
func (h *MessageHandler) Handle(ctx context.Context, input HandleMessageInput) error {
	...
	messages, err := h.buildMessages(ctx, conversationID, systemPrompt, msgContent)
	if err != nil {
		return err
	}
	...
	if h.compactionSvc != nil && h.messageRepo != nil && h.runner.aiProvider != nil {
		...
		if h.compactionSvc.ShouldCompact(messages, maxTokens) {
			...
			messages, _ = h.buildMessages(ctx, conversationID, systemPrompt, msgContent)
			messages = injectMemoryTurn(messages)
			messages = injectSpeakerTurn(messages)
		}
	}
	...
	if senderLabel != "" {
		ctxWithUser = context.WithValue(ctxWithUser, mcp.ContextKeyUserDisplayName, senderLabel)
	}
...
func (h *MessageHandler) buildMessages(ctx context.Context, conversationID, systemPrompt, latestContent string) ([]ports.ChatMessage, error) {
	if h.compactionSvc != nil {
		msgs, err := h.compactionSvc.BuildMessages(ctx, conversationID, systemPrompt)
		if err != nil {
			return nil, err
		}
		if len(msgs) == 0 || msgs[len(msgs)-1].Content != latestContent {
			msgs = append(msgs, ports.ChatMessage{Role: "user", Content: latestContent})
		}
		return msgs, nil
	}
	...
	if len(messages) == 0 || messages[len(messages)-1].Content != latestContent {
		messages = append(messages, ports.ChatMessage{Role: "user", Content: latestContent})
	}
	return messages, nil
}
