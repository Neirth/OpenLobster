...
const internalAgentChannelID = "agent"
...
type HandleMessageInput struct {
	...
	SystemPrompt string
	// OnAssistantResponse, when set, is invoked with the final assistant reply
	// generated by the agentic loop before outbound delivery.
	OnAssistantResponse func(content string)
	// FromDashboard marks messages initiated by the dashboard while preserving
	// platform-native sender/channel identifiers for routing consistency.
	FromDashboard bool
}
...
func isEphemeralInput(input HandleMessageInput) bool {
	return strings.EqualFold(strings.TrimSpace(input.ChannelType), "loopback") ||
		strings.EqualFold(strings.TrimSpace(input.ChannelID), internalAgentChannelID)
}

// messageRoutingKey identifies a message stream for worker locking and queue
// draining. It scopes by channel type plus sender (or channel fallback) so
// simultaneous traffic from different platforms cannot collapse into one lane.
func messageRoutingKey(input HandleMessageInput) string {
	id := strings.TrimSpace(input.SenderID)
	if id == "" {
	...
	ct := strings.ToLower(strings.TrimSpace(input.ChannelType))
	if ct == "" {
		return id
	}
	return ct + "::" + id
...
// Ephemeral internal channels (loopback/agent) are never drained.
func (q *jobQueue) drainUser(pairKey string) *convJob {
	...
	remaining := q.jobs[:0]
	for i := range q.jobs {
		if isEphemeralInput(q.jobs[i].inp) {
			remaining = append(remaining, q.jobs[i])
			continue
		}
		jKey := messageRoutingKey(q.jobs[i].inp)
		if jKey == pairKey {
			if latest != nil {
				latest.done <- nil
			}
...
func (h *MessageHandler) runWorker() {
	for {
		...
		pairKey := messageRoutingKey(job.inp)
		if pairKey == "" {
			pairKey = strings.TrimSpace(job.inp.ChannelID)
		}
...
// Ephemeral internal channels (loopback/agent) are never drained by drainUser.
...
func (h *MessageHandler) handle(ctx context.Context, input HandleMessageInput) error {
	...
	isAgentChannel := strings.EqualFold(strings.TrimSpace(input.ChannelID), internalAgentChannelID)
	isEphemeral := isEphemeralInput(input)
	isDashboardChannel := strings.EqualFold(strings.TrimSpace(input.ChannelType), "dashboard")
	isDashboardOrigin := input.FromDashboard || isDashboardChannel
	isInternal := isEphemeral || isDashboardChannel
	...
	if !isInternal && !isDashboardOrigin && h.channelChecker != nil {
	...
	senderUserID := ""
	if h.channelChecker != nil && !isDashboardOrigin {
	...
	if isDashboardOrigin && !isAgentChannel && input.ConversationID != nil && *input.ConversationID != "" {
		conversationID = *input.ConversationID
		parsed, err := uuid.Parse(conversationID)
		if err != nil {
			return nil
		}
		if h.sessionRepo != nil && !isEphemeral {
			if existing, getErr := h.sessionRepo.GetByID(ctx, conversationID); getErr == nil && existing != nil {
				session = existing
			}
		}
		if session == nil {
	...
	} else if !isEphemeral && h.sessionRepo != nil {
	...
	if senderUserID == "" && session != nil {
		senderUserID = session.UserID
	}

	if session == nil && !isDashboardChannel {
	...
	} else if session != nil && senderUserID != "" && session.UserID == "" && !isDashboardChannel {
		session.UserID = senderUserID
		if h.sessionRepo != nil && !isEphemeral {
	...
	if h.channelChecker != nil {
		...
		if senderLabel == "" && senderUserID != "" && senderUserID != "dashboard" {
			senderLabel, _ = h.channelChecker.GetDisplayNameByUserID(ctx, senderUserID)
		}
		...
		if platformUserID != "" && !isDashboardOrigin {
