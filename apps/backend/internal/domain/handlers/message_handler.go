...
type HandleMessageInput struct {
	...
	// FromDashboard indicates the message originated from the admin dashboard
	FromDashboard bool
}
...
func (h *MessageHandler) handle(ctx context.Context, input HandleMessageInput) error {
	...
	isDashboard := input.ChannelType == "dashboard" || input.FromDashboard
	isInternal := isEphemeral || isDashboard
	...
	if isDashboard && !isAgentChannel && input.ConversationID != nil && *input.ConversationID != "" {
