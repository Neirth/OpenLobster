...
const FirstBootWizard: Component<FirstBootWizardProps> = (props) => {
    ...
    try {
      ...
      if (config) {
        setFormValues({
          ...
          capabilities: {
            ...(getDefaultFormValues().capabilities ?? {}),
  ...
  const handleSaveAndFinish = async () => {
    try {
      ...
      const enabled = (values.plugins as Record<string, Record<string, boolean>>)?.enabled || {};
      const settings = (values.plugins as Record<string, Record<string, Record<string, unknown>>>)?.settings || {};
